...
class DefaultInpaintingTrainingModule(BaseInpaintingTrainingModule):
    ...
    def forward(self, batch):
        ...
        if batch['predicted_image'].shape != batch['image'].shape:
            desired_size = tuple(batch['image'].shape[-2:])
            batch['predicted_image'] = torch.nn.functional.interpolate(batch['predicted_image'], size=desired_size, mode='bilinear', align_corners=False)
